...
const CrashGame = {
  ...
  /**
   * Множитель → вертикаль (логарифмическая шкала).
   * Потолок шкалы подстраивается под полёт: пока множитель ниже 50× — это 50×,
   * выше — шкала расширяется, и ракета не «прилипает» к верхней кромке.
   */
  yTop() {
    const peak = Math.max(
      Number(this.mult) || 1,
      this.phase === 'crashed' ? (Number(this.crashAt) || 1) : 1,
      1
    );
    let top = 50;
    while (peak > top && top < CRASH_CONFIG.maxMultiplier) top *= 2;
    return top;
  },

  /** Отметки сетки высот для текущего потолка шкалы */
  yMarks(top) {
    const marks = [2, 5, 10, 25, 50].filter(m => m < top).slice(0, 4);
    marks.push(top);
    return marks;
  },

  yFor(mult, h, pad, top) {
    const t = Number(top) > 1 ? Number(top) : 50;
    const norm = Math.log(Math.max(1, Number(mult) || 1)) / Math.log(t);
    return h - pad - Math.min(1, Math.max(0, norm)) * (h - pad * 2);
  },

  /**
   * Время набора высоты (τ, без фазы разгона) → горизонталь.
   * Окно фиксированное (windowSec): пока полёт короче окна, ракета ЕДЕТ слева
   * направо по всему полю; когда длиннее — график прокручивается, и ракета
   * остаётся у правого края (как в настоящих crash-играх).
   */
  xFor(tau, tauNow, w, pad) {
    const win = Math.max(2, Number(CRASH_CONFIG.windowSec) || 9);
    const from = Math.max(0, Math.max(win, Number(tauNow) || 0) - win);
    const rel = Math.min(win, Math.max(0, (Number(tau) || 0) - from));
    return pad + (rel / win) * (w - pad * 2);
  },
  ...
  explode() {
    ...
    this.spawnParticles();
    Fx.thunder();
    this.draw();                       // сразу прячем спрайт: частицы могут и не появиться
  ...
  frame() {
    if (this.phase === 'flying') {
      ...
      if (t >= CRASH_CONFIG.takeoffSec) {
        /* ВАЖНО: сравниваем ВРЕМЯ событий, а не множитель. Иначе при заморозке
           вкладки (браузер минуту не звал rAF) один большой шаг времени
           выдавал «автовывод» в раунде, где ракета уже взорвалась — и наоборот.
           Точка краша известна заранее, поэтому честно: что раньше по времени,
           то и случилось. При равенстве побеждает краш. */
        const tCrash = crashTimeToMultiplier(this.crashAt);
        const tAuto = this.autoX ? crashTimeToMultiplier(this.autoX) : Infinity;
        if (t >= Math.min(tCrash, tAuto)) {
          if (tAuto < tCrash) {
            this.mult = this.autoX;
            this.updateHud();
            this.draw();
            this.cashout(true);
          } else {
  ...
  spawnParticles() {
    ...
    const n = low ? 16 : 40;
    const pad = 30;
    const tauCrash = crashGrowthTime(crashTimeToMultiplier(this.crashAt));
    const x = this.xFor(tauCrash, tauCrash, this.w, pad);
    const y = this.yFor(this.crashAt, this.h, pad, this.yTop());
    for (let i = 0; i < n; i++) {
  ...
  /* ======================================================================
     РАКЕТА: SVG-спрайт над канвасом (+ векторный фолбэк внутри canvas)

     Раньше ракета рисовалась только внутри canvas и «терялась»: мелкий вектор
     у правой кромки, который к тому же пересчитывался каждый кадр вместе с
     растягивающимся графиком. Теперь она — отдельный DOM-элемент
     (#crashRocket) с заливным SVG-спрайтом: чёткая на любом экране и DPR,
     всегда выше графика, видна даже если canvas недоступен. Координаты
     считаются ровно из той же кривой, по которой рисуется график, поэтому
     ракета летит по траектории, а не «приклеена» к краю.
     ====================================================================== */
  rocketEl() {
    if (!this._rocketEl) this._rocketEl = $('crashRocket');
    return this._rocketEl;
  },

  /**
   * Ставит спрайт в точку (px, py) и поворачивает по направлению полёта.
   * mode: 'parked' | 'takeoff' | 'flying' | 'landed'.
   * @returns {boolean} false, если DOM-элемента нет (тогда рисуем вектор в canvas)
   */
  placeRocket(px, py, tilt, mode) {
    const el = this.rocketEl();
    if (!el) return false;
    const size = el.offsetWidth || 38;
    const half = size / 2;
    const w = this.w || 320, h = this.h || 210;
    // Спрайт всегда целиком внутри поля — у кромок слегка «притормаживает»
    const x = Math.max(half + 1, Math.min(w - half - 1, px));
    const y = Math.max(half * 0.55, Math.min(h - half * 0.3, py));
    /* Наклон — по касательной к траектории, но сжатый: график на старте почти
       горизонтальный (множитель растёт медленно), и «честные» 80° положили бы
       ракету на бок. Шкала: 0° = вверх, 45° = максимум (вправо-вверх). */
    const raw = Math.max(0, Math.min(90, tilt * 180 / Math.PI));
    const deg = 45 * Math.pow(raw / 90, 0.6);
    el.style.transform = `translate3d(${x.toFixed(1)}px, ${y.toFixed(1)}px, 0) rotate(${deg.toFixed(1)}deg)`;
    el.classList.remove('hidden');
    el.classList.toggle('crash-rocket-flying', mode === 'flying');
    el.classList.toggle('crash-rocket-takeoff', mode === 'takeoff');
    el.classList.toggle('crash-rocket-parked', mode === 'parked' || mode === 'landed');
    return true;
  },
  ...
  /** Наклон ракеты — по касательной к кривой в текущей точке */
  tiltAt(tauEnd, tauNow, w, h, pad, top) {
    const TO = CRASH_CONFIG.takeoffSec;
    const back = 0.3;
    const a = Math.max(0, tauEnd - back);
    const x1 = this.xFor(a, tauNow, w, pad);
    const y1 = this.yFor(crashMultiplierAt(a + TO), h, pad, top);
    const x2 = this.xFor(tauEnd, tauNow, w, pad);
    const y2 = this.yFor(crashMultiplierAt(tauEnd + TO), h, pad, top);
    const dx = x2 - x1, dy = y2 - y1;
    if (!dx && !dy) return 0;
    return Math.atan2(dy, dx) + Math.PI / 2;
  },

  /** Векторная ракета внутри canvas — фолбэк, если DOM-спрайт недоступен */
  drawRocket(x, y, opts = {}) {
    const ctx = this.ctx;
    if (!ctx) return;
    const scale = opts.scale || 1;
    const tilt = Math.max(-0.8, Math.min(0.8, opts.tilt || 0));
  ...
  /** Мягкое свечение в точке ракеты — и под спрайтом, и под векторным фолбэком */
  drawRocketGlow(x, y, rgb) {
    const ctx = this.ctx;
    if (!ctx) return;
    const r = 26;
  ...
  draw() {
    const ctx = this.ctx;
    const w = this.w || 320, h = this.h || 210, pad = 30;
    ...
    /* Горизонталь считаем по «чистому» времени набора высоты (τ):
       во время гарантированного разгона ракета стоит на площадке. */
    const tauNow = crashGrowthTime(this.elapsed());
    const tauEnd = this.phase === 'idle'
      ? 0
      : (flying ? tauNow : crashGrowthTime(crashTimeToMultiplier(this.mult)));

    const px = this.xFor(tauEnd, tauNow, w, pad);
    const py = this.yFor(crashMultiplierAt(tauEnd + CRASH_CONFIG.takeoffSec), h, pad, top);
    const tilt = tauEnd > 0.05 ? this.tiltAt(tauEnd, tauNow, w, h, pad, top) : 0;

    /* ---- Ракета: ставим спрайт ДО рисования канваса (виден даже без canvas) ---- */
    if (crashed) {
      this.hideRocket();
    } else if (this.phase === 'idle') {
      this.placeRocket(pad + 4, h - pad - 2, 0, 'parked');       // стоит на площадке
    } else {
      const mode = takeoff ? 'takeoff' : (flying ? 'flying' : 'landed');
      // На разгоне ракета дрожит на площадке — видно, что двигатель работает
      const shake = takeoff ? Math.sin(this.elapsed() * 26) * 1.6 : 0;
      const ok = this.placeRocket(px, py - shake, tilt, mode);
      if (!ok) this._rocketFallback = { px, py: py - shake, tilt, mode }; // спрайта нет — нарисуем вектор
    }

    if (!ctx) return;                                             // canvas недоступен — спрайт уже на месте
    ...
    /* Сетка высот: 2× / 5× / 10× / … / потолок шкалы */
    ctx.font = '9px Chakra Petch, sans-serif';
    ctx.textBaseline = 'middle';
    for (const mark of this.yMarks(top)) {
      const y = this.yFor(mark, h, pad, top);
      if (y < pad - 10 || y > h - pad + 10) continue;
      ...
      ctx.textAlign = 'right';
      ctx.fillText(`${mark}×`, pad - 17, y);
    }

    /* Траектория полёта */
    const curveColor = crashed ? '#ef4444' : (won ? '#22c55e' : '#38bdf8');
    const win = Math.max(2, Number(CRASH_CONFIG.windowSec) || 9);
    const from = Math.max(0, Math.max(win, tauNow) - win);

    if (this.phase !== 'idle' && tauEnd > 0) {
      const steps = 72;
      ctx.beginPath();
      for (let i = 0; i <= steps; i++) {
        const tau = from + (tauEnd - from) * (i / steps);
        const m = crashMultiplierAt(tau + CRASH_CONFIG.takeoffSec);
        const cx = this.xFor(tau, tauNow, w, pad);
        const cy = this.yFor(m, h, pad, top);
      ...
      /* Заливка под кривой */
      ctx.lineTo(this.xFor(tauEnd, tauNow, w, pad), h - pad);
      ctx.lineTo(this.xFor(from, tauNow, w, pad), h - pad);
      ctx.closePath();
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(pad - 14, h - pad);
    ctx.lineTo(w - pad + 12, h - pad);
    ctx.stroke();

    /* Ракета: свечение + (если нет DOM-спрайта) векторный фолбэк */
    if (!crashed) {
      const mode = takeoff ? 'takeoff' : (flying ? 'flying' : 'landed');
      const showPad = this.phase === 'idle';
      const rx = showPad ? pad + 4 : Math.max(pad, Math.min(w - pad, px));
      const ry = showPad ? h - pad - 2 : py;
      const glow = crashed ? '239,68,68' : (won ? '34,197,94' : '56,189,248');
      if (this.phase !== 'idle') this.drawRocketGlow(rx, ry, glow);

      if (this._rocketFallback) {
        const fb = this._rocketFallback;
        this.drawRocket(fb.px, fb.py, {
          scale: Math.max(1, Math.min(1.7, w / 300)),
          tilt: takeoff ? 0 : fb.tilt,
          flame: flying && !takeoff
        });
        this._rocketFallback = null;
      }
    }
